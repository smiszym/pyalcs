# It is unclear yet if I need this class to implement PEE.
#
# First, I started without a specific class, and simply used dict objects
# as probability-enhanced attributes. But it wouldn't be easy to program
# complex operations (inserting, removing, adjustment of characters),
# so I decided to sacrifice performance for code readability (at least
# for now, and I'm even unsure if the performance impact will be high).
...
class ProbabilityEnhancedAttribute(dict):
    ...
    def merged_attributes(cls, attr1, attr2,
        ...
        """
        # Wildcard is not allowed in any attribute
        result = attr1.copy() \
    ...
    def insert_attribute(self, other, q1, q2):
        ...
        for symbol in self.symbols_specified().union(other.symbols_specified()):
    ...
    def __str__(self):
        if DETAILED_PEE_ATTR_PRINTING:
            return "{" + ", ".join(
                "%s:%.0f%%" % (sym, self[sym] * 100) for sym in self) + "}"
        else:
